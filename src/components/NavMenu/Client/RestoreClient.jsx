...
import { getSoftDeleteClinetsList } from "@/redux/slices/Client/GetSoftDeleteClientList";
...
const RestoreClient = () => {
  ...
  //   }, []);
  const { deletedClientList } = useSelector(
    (state) => state.GetSoftDeleteClientList
  );
  // ----creating data --------
  useEffect(() => {
    dispatch(getSoftDeleteClinetsList());
  }, [dispatch]);

  const data = deletedClientList?.data?.listClients?.map((item) => ({
    ...item,
  ...
  const handlePermanentDelete = (row) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(permanentDeleteClient(id))
          .then((res) => {
            dispatch(getSoftDeleteClinetsList());
            toast.success(res.payload.data.message);
          })
  ...
  useEffect(() => {
    setRecords(
      deletedClientList?.data?.listClients?.map((item) => ({
        ...item,
