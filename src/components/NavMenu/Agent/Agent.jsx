...
const Agent = () => {
  const { agentList } = useSelector((state) => state.AgentSlice);
  ...
  const data = agentList?.data?.listAgents?.map((item) => ({
    ...item,
  ...
  const handleEdit = (row) => {
    const id = row._id;
    navigate("/layout/edit-agents", { state: { id } });
  };
  ...
  const handleInactive = (row) => {
    const id = row._id;
    const status = false;
    dispatch(agentInActive({ id, status }))
      .then((res) => {
        dispatch(agent());
        toast.success(res.payload.data.message);
      })
  ...
  const subHeaderComponent = (
    <div className="w-full grid xl:grid-cols-2  gap-3  items-center justify-between">
      {/* <div className="text-2xl text-start">Agents</div> */}
      {/* Adding Drop Down of Branch list */}
      <div>
        <p className="text-start"> Agents</p>
        {/* <select
          // value={branchId}
          name="branchId"
          // label="State"
