...
const Employee = () => {
  ...
  const handleView = (row) => {
    const id = row._id;
    console.log(id);
    navigate("/layout/view-employee", { state: { id, pathName } });
  };
  const handleEdit = (row) => {
    console.log(row, "row");
    const id = row._id;
    console.log(id, "id");
    navigate("/layout/edit-employee", { state: { id } });
  };
  ...
  const handleInactive = (row) => {
    const id = row._id;
    const status = false;
    dispatch(employeeInActive({ id, status }))
      .then((res) => {
        dispatch(employee());
        toast.success(res.payload.data.message);
      })
  ...
  useEffect(() => {
    setRecords(
      employeeList?.data?.listEmployees?.map((item) => ({
        ...item,
  ...
  const handleFilter = (e) => {
    ...
    setTimeout(() => {
      dispatch(filterEmployee(newkeyWord))
        .then((res) => {
          setFilterData(res.payload.data.listEmployees);
        })
  ...
  useEffect(() => {
    const fetchData = async (page) => {
      ...
      try {
        const response = await dispatch(
          employeePagination({ page, keyWord, perPage })
        );
        console.log(response);
        const data = response.payload.data.listEmployees;
    ...
    fetchData(page);
  }, [employeeList, setPaginationData]);

  // ------Performing Action when page change -----------
  const handlePageChange = (page) => {
    dispatch(employeePagination({ page, keyWord, perPage }))
      .then((res) => {
  ...
  const subHeaderComponent = (
    <div className="w-full grid xl:grid-cols-2  gap-3  items-center justify-between">
      {/* <div className="text-2xl text-start">Employees</div> */}
