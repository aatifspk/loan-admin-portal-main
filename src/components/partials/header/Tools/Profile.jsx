...
const profileLabel = () => {
  ...
  // const verifyAuth = useSelector((state) => state.viewUserSlice);

  const { data, profileCreated } = useSelector((state) => state.viewUserSlice);
  const ProfileSlice = useSelector((state) => state.AuthSlice);
  // const profileCreated = ProfileSlice?.data?.data?.adminInfo?.profileCreated;
    ...
    <div className="flex items-center">
      <div className="flex-1 ltr:mr-[10px] rtl:ml-[10px]">
        <div className="lg:h-8 lg:w-8 h-7 w-7 rounded-full">
          <img
            // src={userDetails.profileImage ?? UserAvatar}
            src={data && profileCreated ? userDetails.profileImage : UserAvatar}
            alt=""
      ...
      <div className="flex-none text-slate-600 dark:text-white text-sm font-normal items-center lg:flex hidden overflow-hidden text-ellipsis whitespace-nowrap">
        <span className="overflow-hidden text-ellipsis whitespace-nowrap w-[85px] block">
          ...
          {data && profileCreated ? userDetails.name : "Admin"}
        </span>
